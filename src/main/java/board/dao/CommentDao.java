...
public class CommentDao {
    ...
    public void deleteComment(Connection conn, int commentNo) throws SQLException {
        ...
        try {
            ...
            String deleteRepliesQuery = "DELETE FROM reply WHERE comment_no = ?";
            pstmt = conn.prepareStatement(deleteRepliesQuery);
            pstmt.setInt(1, commentNo);
            pstmt.executeUpdate();
            JdbcUtil.close(pstmt);

          
            String deleteCommentQuery = "DELETE FROM comment WHERE comment_no = ?";
